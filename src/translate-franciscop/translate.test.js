...
describe('Main', () => {
  beforeEach(() => {
    mock(/googleapis.*target=es/, {
      data: {translations: [{translatedText: 'Hola mundo'}]}
    });
    mock(/googleapis.*target=ja/, {
...
describe('language parsing', () => {
  it('loads', () => {
    expect(translate.language).toBeDefined();
  });
  ...
  it('works with different casing', () => {
    expect(language('spanish')).toBe('es');
    expect(language('SpANisH')).toBe('es');
    expect(language('SPANISH')).toBe('es');
  });
...
describe('Google', () => {
  beforeEach(() => {
    mock(/googleapi.*error/, {
      error: {errors: [{message: 'Google API Error'}]}
    });
    mock(/googleapi.*throw/, {code: 500, message: 'also fails harder'}, true);
    mock(/googleapi.*&target=ru/, {
      data: {translations: [{translatedText: 'Hola mundo'}]}
    });
  });
